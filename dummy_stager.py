...
def root(request):
  ...
  try:
    ...
    return json.dumps({'status': 'Invalid parameters. A job_id, product_id, '
                      'callback, and an authcode (for the callback to pass along) '
                      'must be specified'})

  # Process request in separate thread
  reactor.callInThread(process_staging_request, job_id, product_id, callback, authcode)
